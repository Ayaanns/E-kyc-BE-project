    ...
    def detect_wave(self, frame):
        ...
        if not results.multi_hand_landmarks:
            if self.show_wave_prompt:
                cv2.putText(frame, "Please wave your hand", (10, 110),
        ...
        while cap.isOpened():
            ...
            if self.showing_excellent:
                if time.time() - self.excellent_start_time <= 3.0:
                    cv2.putText(frame, "Excellent!", (10, 70),
                              cv2.FONT_HERSHEY_SIMPLEX, 1, self.TEXT_COLOR, 2)
                else:
                    self.showing_excellent = False
                ...
                if self.current_phase == "wave":
                    cv2.putText(frame, f"Waves completed: {self.wave_count}/2", (10, 30), 
                               cv2.FONT_HERSHEY_SIMPLEX, 1, self.TEXT_COLOR, 2)
                    
                    if self.wave_count > 0 and self.wave_count < 2 and self.show_wave_prompt:
                        cv2.putText(frame, "Please wave your hand", (10, 70),
                                   cv2.FONT_HERSHEY_SIMPLEX, 1, self.TEXT_COLOR, 2)
                else:
                    cv2.putText(frame, f"Blinks detected: {self.blink_counter}/3", (10, 30),
                               cv2.FONT_HERSHEY_SIMPLEX, 1, self.TEXT_COLOR, 2)
                    
                    if not self.blinks_verified:
                        cv2.putText(frame, "Please blink your eye", (10, 70),
                                  cv2.FONT_HERSHEY_SIMPLEX, 1, self.TEXT_COLOR, 2)

            if self.blinks_verified and self.wave_phase_complete:
                if not self.photo_prompt_shown:
                    cv2.putText(frame, "Kindly move to a clear background for photo capture", (10, 120),
                               cv2.FONT_HERSHEY_SIMPLEX, 1, self.TEXT_COLOR, 2)
                    cv2.putText(frame, "Press SPACE when ready", (10, 160),
                               cv2.FONT_HERSHEY_SIMPLEX, 1, self.TEXT_COLOR, 2)
                ...
                if key == 32:  # SPACE key
                    ...
                    if remaining_time > 0:
                        cv2.putText(frame, f"Photo in {remaining_time}...", (10, 200),
                                   cv2.FONT_HERSHEY_SIMPLEX, 1, self.TEXT_COLOR, 2)
                    else:
                        # Capture and save photo
                        self.save_photo(frame)
                        cv2.putText(frame, "Photo captured!", (10, 200),
                                   cv2.FONT_HERSHEY_SIMPLEX, 1, self.TEXT_COLOR, 2)
